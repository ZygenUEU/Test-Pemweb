...
@section('content')
    ...
    @can('mobilegame_create')
        <div style="margin-bottom: 10px;" class="text-white">
            <a href="{{ route('admin.mobilegames.create') }}" class="add-btn text-center">
    ...
    <div class="card-columns mt-5">
        @foreach ($mobilegames as $mobilegame)
            <div class="card card-item text-white border-0" data-card-id="{{ $mobilegame->id }}"  style="background-color: #202020;">
                ...
                <a class="text-decoration-none text-white" href="{{ route('admin.mobilegames.show', $mobilegame->id) }}">
                    <h5 class="card-title ml-3 mr-3">{{ $mobilegame->gamename }}</h5>
                </a>
                ...
                <div class="card-body card-item-body">
                    ...
                    <div class="row" style="margin-bottom: -10px;">
                        ...
                        <div class="col-md-6">
                            <p class="card-dev" style="float: right; text-align: right">{{ $mobilegame->developer }}</p>
                        </div>
                    </div>
                    ...
                    <div class="row" style="margin-bottom: -10px;">
                        ...
                        <div class="col-md-6">
                            <p class="card-genre" style="float: right; text-align: right">{{ $mobilegame->genre }}</p>
                        </div>
                    </div>
                    ...
                    <div class="row" style="margin-bottom: -10px;">
                        ...
                        <div class="col-md-6">
                            <p style="float: right; text-align: right">{{ $mobilegame->publish_date }}</p>
                        </div>
                    </div>


                    <div>
                        {{-- <a href="{{ $mobilegame->download_url }}" class="w-100 btn btn-primary mt-3">{{ trans('global.download') }}</a> --}}

                        <br>

                        @can('mobilegame_edit')
                            @if ($mobilegame->postedby === Auth::user()->name || Auth::user()->name === 'Admin')
                                <a class="w-100 btn btn-info mt-3" href="{{ route('admin.mobilegames.edit', $mobilegame->id) }}">
                                    {{ trans('global.edit') }}
                                </a>
                            @endif
                        @endcan
                        ...
                        @can('mobilegame_delete')
                            <form action="{{ route('admin.mobilegames.destroy', $mobilegame->id) }}" method="POST"
        ...
        $(function() {
            ...
            @can('mobilegame_delete')
                ...
                let deleteButton = {
                    ...
                    url: "{{ route('admin.mobilegames.massDestroy') }}",
            ...
            let dtOverrideGlobals = {
                ...
                retrieve: true,
                aaSorting: [],
                ajax: "{{ route('admin.mobilegames.index') }}",
                columns: [{
                        data: 'placeholder',
                        name: 'placeholder'
                    },
